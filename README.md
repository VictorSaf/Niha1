...
- **User Authentication** - Secure JWT-based authentication with optimized navigation flow and role-based redirects. The window scrolls to top on every route change so each new page opens at the top. Users in **NDA** or **KYC** roles can access only onboarding (`/onboarding`, sub-routes, `/onboarding1`, `/learn-more`) and public routes; all other protected routes redirect them to `/onboarding`. **Introducer role** — Dedicated entry at `/introducer` (ENTER + NDA). Users approved from Introducer NDA requests get role **INTRODUCER** and access only to a simplified dashboard at `/introducer/dashboard` (no cash market, swap, or funding). Backoffice → Onboarding → **Introducer** tab for approving introducer requests. Login page offers ENTER (password) and NDA (request access); NDA flow supports **"I have a code"** (referral code) → introducer form. With a valid PREINTRODUCER code and no NDA file, the backend creates an **INTRODUCER** user and sends an email with NDA attached and a setup-password link; after setting password the user is redirected to **/introducer/dashboard** (no NDA upload or admin approval). Otherwise a contact request is created (PREINTRODUCER flow; admin approves NDA in Backoffice). After NDA submit, a "Request Submitted" confirmation is shown, then after 5 seconds the content fades out and an ambient animation is displayed. Preview: `/login?preview=nda-success` shows the NDA success flow without submitting.
...
#### Pages:
- **Onboarding** (default) - Contact Requests, **Introducer**, KYC Review, Deposits (SubSubHeader nav; route-based content). **Introducer** tab shows pending requests with `request_flow='introducer'` (data lives in `contact_requests`, not only in `users`); Approve & Create User with `target_role=INTRODUCER` creates users without Entity. If introducer users are removed via SQL maintenance, stale introducer rows may remain until **Reconcile orphans** on the Introducer tab, **`POST /api/v1/admin/contact-requests/reconcile-introducer-orphans`**, or **`scripts/sql/reconcile_introducer_orphan_contact_requests.sql`** (see **`app_truth.md`** §8 and **`docs/API.md`**). Contact Requests: list shows only **pending** requests (NDA/new); approved (KYC) or rejected disappear immediately. Entity and Name per row (with "—" when missing); list and badge update in real time via WebSocket. Compact list rows with View modal (all contact request fields, NDA open-in-browser button), **Approve & Create User** (manual or invitation; creates entity + KYC user, sets request to KYC), Reject, Delete; IP lookup available from View modal
...
- **Users** - User management (accessible from backoffice Subheader nav). List and user detail modal show Status as **Active** or **DISABLED** (based on `is_active`); Role column shows DISABLED badge for disabled users. From User Detail, admins can adjust entity balances via **Add Asset** (Deposit / Withdraw) for EUR, CEA, or EUA. **Deposit & Withdrawal History** (Assets tab) shows a unified list of wire deposits and add-asset transactions, sorted by date. Add-asset operations create audit tickets visible in Logging. **Pre-Introducer** creation via **Create User** sends the **introducer_nda_invitation** email (NDA PDF + setup-password link) and creates a pending Introducer onboarding row; see **`docs/API.md`** (`POST /admin/users/create-preintroducer`) and **`app_truth.md`** §8.
